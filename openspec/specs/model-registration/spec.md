...
- **THEN** calling `register()` registers the post type with those arguments
...
The library SHALL provide `AbstractBlocks` that discovers and registers blocks from paths defined by the consumer.
...
The library SHALL provide `Loader` that collects actions and filters and applies them.
...
The library SHALL provide `BootstrapRunner` with a static `run()` method to instantiate Definition components and register them.

#### Scenario: Runner registers model components
- **WHEN** a consumer calls `BootstrapRunner::run(Page\Bootstrap::class, Event\Bootstrap::class)`
